...
- **Persistence — built to survive** — every save is written to **both localStorage and IndexedDB** in a timestamped envelope; loads pick the newest valid copy and self-heal the other engine. A rolling previous-value snapshot survives corrupted writes, and the app requests `navigator.storage.persist()` so browsers avoid evicting it. **Export / Import backup** (on the Pantry page) gives you a JSON file that's yours forever — importing *merges*, never deletes.

### Keeping your pantry on mobile (important)

- **Use the hosted HTTPS link** (GitHub Pages). Opening the downloaded `index.html` directly on a phone runs it in a throwaway file sandbox — nothing can persist there, and the app shows a warning banner when it detects this.
- **Avoid private/incognito tabs** — those wipe all site data on close by design.
- **iOS Safari** deletes a site's data after ~7 days without a visit. Visiting occasionally resets the clock; adding the site to your **Home Screen** (Share → Add to Home Screen) exempts it.
- For an ironclad copy regardless of any of the above: **Export backup**, keep the file, import anywhere.
